...
const Sprint = ({ words, onSetPage, onSetAnswers }: IProps) => {
  ...
  const handleIncorrect = () => {
    ...
    newArr.unshift(
      <div>
        Неверно: {word} переводится как {correctTransl}
      </div>,
    );
    setLastWord(newArr);
  ...
  const handleCorrect = () => {
    ...
    newArr.unshift(
      <div>
        Верно: {word} переводится как {correctTransl}
      </div>,
    );
    setLastWord(newArr);
  ...
  const handleSetTask = () => {
    ...
    correctTranslation
      ...
      : setTranslation(
          words[handleGetIncorrectTranslation(currindex)].wordTranslate,
        );
  };
    ...
    <div className={s.root}>
      ...
      <div className={s[containerClass]}>
        ...
        <div className={s.header} style={{ backgroundColor: color }}>
          <div className={s.streak}>{streakRender()}</div>
          <span className={s.factor}>+ {factor * 10} очков за слово</span>
        </div>
        <div className={s.timer}>{countDown}</div>
        <span className={s.word}>{word || ''}</span>
        <span className={s.translation}>{translation || ''}</span>

        <div className={s.buttons}>
          <button type="button" onClick={() => handleAnswerButtons(false)}>
            Неверно
          </button>

          <button type="button" onClick={() => handleAnswerButtons(true)}>
            Верно
          </button>
        </div>
      </div>
